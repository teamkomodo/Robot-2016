...
public:
	...
	Robot() :
		...
		encoderM(ENCODER_CHANNEL_MA, ENCODER_CHANNEL_MB), //Ball manipulator encoder
		encoderU(ENCODER_CHANNEL_UA, ENCODER_CHANNEL_UB), //Lifter encoder
		lifter(LIFTER_CHANNEL_EXTEND, LIFTER_CHANNEL_TILT, encoderU),
		ballManipulator(B_MANIPULATOR_CHANNEL_LIFT, B_MANIPULATOR_CHANNEL_PINCH, OPEN_SWITCH, CLOSE_SWITCH, encoderM)
	{
...
void Robot::AutonomousPeriodic() {
	...
	if (fabs(encoder1.GetRaw()) < ONE_FOOT_LEFT_ENCODER*3 && encoder2.GetRaw() < ONE_FOOT_RIGHT_ENCODER*3)
		DriverControl(AUTOBOT, 0.5, autoTurn);
	else
...
void Robot::TeleopInit() {
	...
	isGyroDrive = false; //SET THIS TO TRUE TO ENABLE TELEOP GYRO DRIVE
	driveOption = TANK_2;
	gyro.Reset();
	encoder1.Reset();
	encoder2.Reset();
...
void Robot::TeleopPeriodic() {
	...
	ButtonControl();
	ballManipulator.Tilt(-B_MANIPULATOR_SCALE_FACTOR*gamePad.GetRawAxis(GAMEPAD_RIGHT_STICK_Y)); //These four functions control the manipulators
	ballManipulator.TempPinch(gamePad.GetRawAxis(GAMEPAD_LEFT_STICK_Y)); //Replace w/ ballManipulator.Pinch();
	//lifter.Tilt(gamePad.GetRawAxis(GAMEPAD_LEFT_STICK_X));
	//lifter.ExtendRetract(-gamePad.GetRawAxis(GAMEPAD_LEFT_STICK_Y));
...
//Buttons on the gamepad control various other things.
	if(gamePad.GetRawButton(GAMEPAD_BUTTON_A)== true && buttonDone[GAMEPAD_BUTTON_A]== false){
		...
		ballManipulator.openFlag = !ballManipulator.openFlag;
		ballManipulator.closeFlag = !ballManipulator.closeFlag;
	...
	if(gamePad.GetRawButton(GAMEPAD_BUTTON_Y)== true && buttonDone[GAMEPAD_BUTTON_Y]== false){
		...
		ballManipulator.pinchFlag = !ballManipulator.pinchFlag;
...
void Robot::PrintStatements() { //Function to print stuff. Makes the code much easier to read through.
	 ...
	 SmartDashboard::PutNumber("Gamepad POV: ", gamePad.GetPOV());
		...
		// Print out the drive motor encoder data
		...
		//Print out the lifter and manipulator encoder data
		if(showArmEncoders == true) {
			...
			SmartDashboard::PutNumber("Lifter Encoder Raw", encoderU.GetRaw());
			SmartDashboard::PutNumber("Lifter Encoder Rate", encoderU.GetRate());
		}
